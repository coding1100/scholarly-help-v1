...
const PAGE_ID = "take-my-class-still-doing";

async function fetchTakeMyClassStillDoingData() {
  try {
    const query = {
...
const Page = async () => {
  const pageData = await fetchTakeMyClassStillDoingData();
  const rawBaseUrl =
  ...
  const baseUrl = rawBaseUrl.endsWith("/")
    ...
    : rawBaseUrl;
  const productTitle =
    pageData?.meta?.title || MetaData.takeMyClassStillDoing.title;
  const metaDescription =
    pageData?.meta?.description || MetaData.takeMyClassStillDoing.description;
  const pageUrl =
    pageData?.meta?.canonicalUrl ||
    `${baseUrl}/${MetaData.takeMyClassStillDoing.url}`;
...
export async function generateMetadata({}): Promise<Metadata> {
  ...
  const canonicalUrl =
    ...
    `${baseUrl}${MetaData.takeMyClassStillDoing.url}`;
  return {
    title: pageData?.meta?.title || MetaData.takeMyClassStillDoing.title,
    description:
      pageData?.meta?.description || MetaData.takeMyClassStillDoing.description,
    alternates: {
