...
const PAGE_ID = "take-my-class-protect-gpa";

async function fetchTakeMyClassProtectGpaData() {
  try {
    const query = {
...
const Page = async () => {
  const pageData = await fetchTakeMyClassProtectGpaData();
  const rawBaseUrl =
  ...
  const baseUrl = rawBaseUrl.endsWith("/")
    ...
    : rawBaseUrl;
  const productTitle =
    pageData?.meta?.title || MetaData.takeMyClassProtectGpa.title;
  const metaDescription =
    pageData?.meta?.description || MetaData.takeMyClassProtectGpa.description;
  const pageUrl =
    pageData?.meta?.canonicalUrl ||
    `${baseUrl}/${MetaData.takeMyClassProtectGpa.url}`;
...
export async function generateMetadata({}): Promise<Metadata> {
  ...
  const canonicalUrl =
    ...
    `${baseUrl}${MetaData.takeMyClassProtectGpa.url}`;
  return {
    title: pageData?.meta?.title || MetaData.takeMyClassProtectGpa.title,
    description:
      pageData?.meta?.description || MetaData.takeMyClassProtectGpa.description,
    alternates: {
