...
const PAGE_ID = "take-my-class-professor-does-not-care";

async function fetchTakeMyClassProfessorData() {
  try {
    const query = {
...
const Page = async () => {
  const pageData = await fetchTakeMyClassProfessorData();
  const rawBaseUrl =
  ...
  const baseUrl = rawBaseUrl.endsWith("/")
    ...
    : rawBaseUrl;
  const productTitle =
    pageData?.meta?.title || MetaData.takeMyClassProfessorDoesNotCare.title;
  const metaDescription =
    pageData?.meta?.description ||
    MetaData.takeMyClassProfessorDoesNotCare.description;
  const pageUrl =
    pageData?.meta?.canonicalUrl ||
    `${baseUrl}/${MetaData.takeMyClassProfessorDoesNotCare.url}`;
