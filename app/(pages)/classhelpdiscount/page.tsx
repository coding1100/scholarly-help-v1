...
const Page: FC<PageProps> = ({}) => {
  ...
  return (
    <>
      <ProductSchema
        productTitle={MetaData.classHelpDiscount.title}
        metaDescription={MetaData.classHelpDiscount.description}
        pageUrl={`${baseUrl}/${MetaData.classHelpDiscount.url}`}
      />
      <MainLayout>
        <Hero />
        <Qualities />
...
export function generateMetadata({}) {
  const baseUrl =
    process.env.NEXT_PUBLIC_SITE_URL || "https://scholarlyhelp.com/";
  const canonicalUrl = `${baseUrl}${MetaData.classHelpDiscount.url}`;
  return {
    title: `${MetaData.classHelpDiscount.title}`,
    description: `${MetaData.classHelpDiscount.description}`,
