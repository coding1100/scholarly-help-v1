 ...
 * the CURRENT document, so accepted fixes are reflected immediately. The PDF
 * itself is rendered by the backend (/tools/grammar-check/report) and arrives
 * as a direct file download — the parent owns that call (it has the auth
 * headers and document), so it's passed in as onDownloadPdf.
 */
    ...
    >
      <div className="max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-6 shadow-xl dark:bg-gray-900 print:max-h-none print:shadow-none">
        ...
        <button
          ...
          className={`mt-5 w-full rounded-lg bg-primary-400 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-primary-300 ${
