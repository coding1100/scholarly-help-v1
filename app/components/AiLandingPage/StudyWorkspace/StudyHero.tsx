...
import StudyToolEmbed from "./StudyToolEmbed";
import { heroContent } from "./content";

/**
 * Hero for the /tools/ai-study-workspace landing page: badge, headline, 3-step
 * indicator, the real study tool embedded as a card, and the stats bar. Mirrors
 * the paraphraser hero's layout and background treatment.
 *
 * This component is a SERVER component so the marketing copy (badge, H1,
 * subheading, steps, stats) is in the server-rendered HTML for SEO. Only the
 * interactive tool — which reads ?sessionId= via useSearchParams and so needs a
 * Suspense boundary — is client-rendered, inside StudyToolEmbed.
 */
const StudyHero: FC = () => (
  <section className="relative overflow-hidden bg-[linear-gradient(180deg,#EAF3FF_0%,#F2F9FF_55%,#F7FBFF_100%)]">
    {/* soft decorative washes matching the paraphraser hero */}
    <div
      aria-hidden
    ...
    <div className="relative mx-auto max-w-[1240px] px-4 pb-16 pt-10 text-center md:pb-20 md:pt-14">
      ...
      {/* the real tool, styled as the Figma hero card */}
      <div
        id="study-workspace-tool"
      ...
      {/* stats bar */}
      <div className="mt-10 flex flex-wrap items-center justify-center gap-x-4 gap-y-3 text-sm text-gray-800 md:gap-x-6 md:text-base">
        {heroContent.stats.map((stat, i) => (
          <div key={stat} className="flex items-center gap-x-4 md:gap-x-6">
            {i > 0 && (
