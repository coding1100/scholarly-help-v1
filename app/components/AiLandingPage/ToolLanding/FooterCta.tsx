...
export interface FooterCtaContent {
  ...
  /** Optional small line under the buttons (e.g. guarantees). */
  footnote?: string;
}

/** Dark patterned closer with a pill word in the heading and two CTAs. */
const FooterCta: FC<{ content: FooterCtaContent }> = ({ content: c }) => (
  <section
  ...
  >
    <div className="mx-auto max-w-[1240px] px-4 text-center">
      ...
      {c.footnote && (
        <p className="mt-8 text-sm text-white/70">{c.footnote}</p>
      )}
    </div>
  </section>
);
